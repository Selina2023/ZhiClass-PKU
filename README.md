...
- 8月9日
    ...
    - homepage的显示方式进行了调整，大家可以运行查看，主要改了```_index.md```的渲染方式，把标题居左并调整了宽度，我觉得这样比较美观。同时homepage的文章卡片是使用[shortcode]()的方式实现的，可以参考```_index.md```中的写法。
    ...
    - 我按照[这篇文章]()的方法修改了源码（**请不要直接更改源码**），更改的文件放在：
